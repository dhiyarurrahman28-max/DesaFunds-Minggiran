...
// Ambil data dusun dan kategori untuk dropdown
try {
    $stmt = $pdo->prepare("SELECT * FROM dusun ORDER BY nama_dusun");
    $stmt->execute();
    $dusun_list = $stmt->fetchAll();
    
    $stmt = $pdo->prepare("SELECT * FROM kategori_forum ORDER BY nama_kategori");
    $stmt->execute();
...
// Handle form submission - Tambah keluhan baru
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_kritik'])) {
    ...
    if (!empty($dusun_id) && !empty($kategori_id) && !empty($nama) && !empty($pesan)) {
        try {
...
// Handle admin reply
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_reply']) && $is_admin_logged_in) {
    ...
    try {
        ...
        header("Location: " . $_SERVER['PHP_SELF'] . "?reply_success=1");
        exit();
    } catch(PDOException $e) {
...
// Handle delete post
if (isset($_GET['delete_id']) && $is_admin_logged_in) {
...
if (isset($_GET['dusun'])) {
    $filter_dusun = clean_input($_GET['dusun']);
}
if (isset($_GET['kategori'])) {
    $filter_kategori = clean_input($_GET['kategori']);
}

// Ambil data forum dengan filter
try {
    $query = "
...
<head>
    ...
    <title>Forum - DesaFunds Minggiran</title>
