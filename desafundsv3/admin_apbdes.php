...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_pemasukan'])) {
    $apbdes_id = clean_input($_POST['apbdes_id']);
    $jenis_pemasukan_id = clean_input($_POST['jenis_pemasukan_id']);
    $jumlah = clean_input($_POST['jumlah']);
    $keterangan = clean_input($_POST['keterangan']);
    $tanggal = clean_input($_POST['tanggal']);
    
    try {
        $stmt = $pdo->prepare("INSERT INTO pemasukan (apbdes_id, jenis_pemasukan_id, jumlah, keterangan, tanggal) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$apbdes_id, $jenis_pemasukan_id, $jumlah, $keterangan, $tanggal]);
        
        // Update total pemasukan di APBDes
        $stmt = $pdo->prepare("UPDATE apbdes SET total_pemasukan = total_pemasukan + ? WHERE id = ?");
        $stmt->execute([$jumlah, $apbdes_id]);
        
        header("Location: admin_apbdes.php?success=1");
        exit();
    } catch(PDOException $e) {
        $error = "Gagal menambah pemasukan: " . $e->getMessage();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_pengeluaran'])) {
    $apbdes_id = clean_input($_POST['apbdes_id']);
    $jenis_pengeluaran_id = clean_input($_POST['jenis_pengeluaran_id']);
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO pengeluaran (apbdes_id, jenis_pengeluaran_id, nama_kegiatan, anggaran, realisasi, sisa_anggaran, keterangan, tanggal) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$apbdes_id, $jenis_pengeluaran_id, $nama_kegiatan, $anggaran, $realisasi, $sisa_anggaran, $keterangan, $tanggal]);
        
        // Update total pengeluaran di APBDes
        $stmt = $pdo->prepare("UPDATE apbdes SET total_pengeluaran = total_pengeluaran + ? WHERE id = ?");
        $stmt->execute([$realisasi, $apbdes_id]);
...
<head>
    ...
    <style>
        ...
        .nav-admin .nav-link.active {
            border-bottom: 3px solid #1966f5;
            color: #1966f5;
        }
        .admin-container {
            background: white;
    ...
    <nav class="navbar navbar-expand-md navbar-custom px-3">
        <a class="navbar-brand d-flex align-items-center fw-bold" href="index.php">
            <div style="width:44px;height:44px;background:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-right:8px;">
                <svg xmlns="http://www.w3.org/2000/svg" fill="#57266a" viewBox="0 0 64 64" width="30" height="30" aria-hidden="true" focusable="false">
                        ...
                        <?php if (!empty($pemasukan)): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="?hapus_pemasukan=<?php echo $item['id']; ?>&tahun=<?php echo $selected_year; ?>&jenis=pemasukan" 
                                       class="btn btn-danger btn-sm btn-action"
                                       onclick="return confirm('Hapus data pemasukan ini?')">Hapus</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                    ...
                    <tbody>
                        <?php if (!empty($pengeluaran)): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-warning btn-sm btn-action">Edit</button>
                                    <a href="?hapus_pengeluaran=<?php echo $item['id']; ?>&tahun=<?php echo $selected_year; ?>&jenis=pengeluaran" 
                                       class="btn btn-danger btn-sm btn-action"
                                       onclick="return confirm('Hapus data pengeluaran ini?')">Hapus</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST">
                    ...
                    <div class="modal-body">
                        <input type="hidden" name="apbdes_id" value="<?php echo $apbdes['id'] ?? ''; ?>">
                        <div class="mb-3">
                            <label class="form-label">Jenis Pemasukan</label>
                            <select class="form-select" name="jenis_pemasukan_id" required>
                                <option value="">Pilih Jenis</option>
                                <?php foreach($jenis_pemasukan as $jenis): ?>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" name="tambah_pemasukan" class="btn btn-primary">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <form method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="jenis_pengeluaran_id" required>
                                <option value="">Pilih Jenis</option>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" name="tambah_pengeluaran" class="btn btn-primary">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
